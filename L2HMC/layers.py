...
class Dense(nn.Module):
    ...
    def __init__(self, in_, out_, scope='dense', factor=1.0):
        ...
        self.fc.weight = nn.Parameter(0.0001*2 * torch.ones((out_, in_), dtype=torchType))
...
class Parallel(nn.Module):
    ...
    def __init__(self, x_dim, factor):
        ...
        self.seq1 = nn.Sequential(
            Dense(x_dim, x_dim, scope='linear_s', factor=0.001),
            ScaleTanh(x_dim, scope='scale_s'))
        
        self.fc_par_1 = Dense(x_dim, x_dim, scope='linear_t', factor=0.001)

        self.seq2 = nn.Sequential(
            Dense(x_dim, x_dim, scope='linear_f', factor=0.001),
...
class Zip(nn.Module):
    ...
    def __init__(self, x_dim, factor):
        ...
        self.fc_zip_1 = Dense(x_dim, x_dim, scope='embed_1', factor=1.0 / 3)
        self.fc_zip_2 = Dense(x_dim, x_dim, scope='embed_2', factor=factor * 1.0 / 3)
...
class Net_old(nn.Module):
    ...
    def __init__(self, x_dim, factor, scope=None):
        ...
        self.fc1 = Dense(x_dim, x_dim, scope='linear_1')
...
class Net(nn.Module):
    ...
    def __init__(self, x_dim, factor, scope=None):
        ...
        self.fc_zip_1 = nn.Linear(x_dim,x_dim )
        nn.init.kaiming_normal_(self.fc_zip_1.weight, mode='fan_in', a=2./3.)
        nn.init.constant_(self.fc_zip_1.bias, 0.)

        self.fc_zip_2 = nn.Linear(x_dim, x_dim)
        nn.init.kaiming_normal_(self.fc_zip_2.weight, mode='fan_in', a=factor*2./3.)
        nn.init.constant_(self.fc_zip_2.bias, 0.)

        self.fc_zip_3 = nn.Linear(2, x_dim)
        nn.init.kaiming_normal_(self.fc_zip_3.weight, mode='fan_in', a=2./3.)
        ...
        self.fc_par_1 = nn.Linear(x_dim, x_dim)
        nn.init.kaiming_normal_(self.fc_par_1.weight, mode='fan_in', a=0.001*2)
        nn.init.constant_(self.fc_par_1.bias, 0.)

        self.scale_1 = nn.Parameter(torch.zeros((1, x_dim)))

        self.fc_par_2 = nn.Linear(x_dim, x_dim)
        nn.init.kaiming_normal_(self.fc_par_2.weight, mode='fan_in', a=0.001*2)
        nn.init.constant_(self.fc_par_2.bias, 0.)

        self.fc_par_3 = nn.Linear(x_dim, x_dim)
        nn.init.kaiming_normal_(self.fc_par_3.weight, mode='fan_in', a=0.001*2)
...
#         out1 = (self.scale_1) * torch.tanh(self.fc_par_1(h4))
        out2 = self.fc_par_2(h4)
        out3 = torch.exp(self.scale_3) * torch.tanh(self.fc_par_3(h4))
#         out3 = (self.scale_3) * torch.tanh(self.fc_par_3(h4))
